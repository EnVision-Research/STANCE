...
from finetune.dataset import I2VDatasetWithResize, T2VDatasetWithResize
from finetune.dataset.utils import (
    load_images,
    load_prompts,
    load_videos,
...
class Trainer:
    ...
    def prepare_dataset(self) -> None:
        ...
        if getattr(self.components, "depth_model", None) is not None:
            self.components.depth_model.eval()
            self.components.depth_model = self.components.depth_model.to(self.accelerator.device)
    ...
    def prepare_trainable_parameters(self):
        ...
        # tianshuo
        if self.args.load_pretrained_weight is not None:
            # assert self.resume_from_checkpoint is None, "Cannot load pretrained weight and resume from checkpoint at the same time."
            logger.info('load pretrained weight, for the first time...')
            path = self.args.load_pretrained_weight
            state_dict = torch.load(path, map_location="cpu")
            self.components.transformer.load_state_dict(state_dict['module'], strict=True)
        
        # For LoRA, we freeze all the parameters
        # For SFT, we train all the parameters in transformer model
        for attr_name, component in vars(self.components).items():
            if hasattr(component, "requires_grad_"):
                if "lora_" in attr_name:
                    component.requires_grad_(True)
                elif self.args.training_type == "sft" and attr_name == "transformer":
                    component.requires_grad_(True)
                else:
    ...
    def prepare_optimizer(self) -> None:
        ...
        # save the names of the parameters that require gradients
        grad_params = []
        for name, param in self.components.transformer.named_parameters():
            if param.requires_grad:
                grad_params.append(name)
        
        with open('params_have_grad.txt', 'w') as f:
            f.write('\n'.join(grad_params))
    ...
    def train(self) -> None:
        ...
        for epoch in range(first_epoch, self.args.train_epochs):
            ...
            for step, batch in enumerate(self.data_loader):
                ...
                module_embed = self.components.transformer.mudulate_embed
                if module_embed is not None and step < 200 and step % 20 == 0 and accelerator.is_main_process:
                    print(module_embed[:, :10])
    ...
    def validate(self, step: int) -> None:
        ...
        accelerator = self.accelerator
        num_validation_samples = self.args.num_validation_samples
        ...
        if self.args.do_validation:
            #num_validation_samples //= self.accelerator.num_processes

            
            # Don't divide by num_processes to ensure we validate all available samples
            # num_validation_samples //= self.accelerator.num_processes
            # Instead, limit by the actual dataset size to avoid errors
            actual_dataset_size = len(self.dataset) if hasattr(self.dataset, '__len__') else num_validation_samples
            num_validation_samples = min(num_validation_samples, actual_dataset_size)

        for sample_index in range(num_validation_samples):  # world size
            ...
            for key, value in list(artifacts.items()):
                ...
                filename = f"validation-{step}-{accelerator.process_index}-{sample_index}.{extension}"
