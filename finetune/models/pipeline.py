...
class CogVideoXImageToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        num_frames = (num_frames - 1) // self.vae_scale_factor_temporal + 2 + 2 
        # +2 because we output seg+flow: (8N+1)*2; +2 because we add 1 frame for padding
        ...
        # image_latents = torch.cat(image_latents, dim=0).to(dtype).permute(2, 0, 1, 3, 4)  # [B, F, C, H, W]
        ...
        image_latents = torch.cat((
            self.vae.encode(image[:, :, 0:1]).latent_dist.sample(),
            self.vae.encode(image[:, :, 1:2]).latent_dist.sample(),
            ), dim=2
        ) * self.vae_scaling_factor_image

        image_latents = image_latents.permute(0, 2, 1, 3, 4)  # from [B, C, F, H, W] to [B, F, C, H, W]
    ...
    def _prepare_rotary_positional_embeddings(
        ...
        if p_t is None:
            ...
            freqs_cos, freqs_sin = get_3d_rotary_pos_embed(
                ...
                temporal_size=num_frames * 2 if condition_latent is not None else num_frames,
                device=device,
            )
        else:
            # CogVideoX 1.5
            ...
            # freqs_cos_0, freqs_sin_0 = get_3d_rotary_pos_embed(
            #     embed_dim=self.transformer.config.attention_head_dim,
            #     crops_coords=None,
            #     grid_size=(grid_height, grid_width),
            #     temporal_size=base_num_frames - 1,
            #     grid_type="slice",
            ...
            freqs_cos_0, freqs_sin_0 = get_3d_rotary_pos_embed(
                ...
                crops_coords=None,
                grid_size=(grid_height, grid_width),
                temporal_size=(base_num_frames - 1) // 2,
                grid_type="slice",
                max_size=(base_size_height, base_size_width),
                device=device,
            )

            freqs_cos_1, freqs_sin_1 = freqs_cos_0.clone(), freqs_sin_0.clone()

            freqs_cos_2, freqs_sin_2 = get_3d_rotary_pos_embed(
                embed_dim=self.transformer.config.attention_head_dim,
                crops_coords=None,
                grid_size=(grid_height, grid_width),
                temporal_size=1,
            ...
            # freqs_cos = torch.cat([freqs_cos_0, freqs_cos_2], dim=0)
            # freqs_sin = torch.cat([freqs_sin_0, freqs_sin_2], dim=0)
    ...
    def __call__(
        ...
        if dense_mask is not None:  # classifier free guidance, tianshuo
            dense_mask = torch.cat([torch.zeros_like(dense_mask), dense_mask]) if do_classifier_free_guidance else dense_mask
        ...
        if instance_flow is not None:
            instance_flow = torch.cat([instance_flow] * 2) if do_classifier_free_guidance else instance_flow
        
        if camera_motion is not None:
            camera_motion = torch.cat([camera_motion] * 2) if do_classifier_free_guidance else camera_motion

        # For CogVideoX 1.5, the latent frames should be padded to make it divisible by patch_size_t
        patch_size_t = self.transformer.config.patch_size_t
        additional_frames = 0
        # tianshuo, the following code is not used
        if patch_size_t is not None:
            additional_frames = 1  # tianshuo
        ...
        image_rotary_emb = (
            self._prepare_rotary_positional_embeddings(
                height, 
                width, 
                latents.size(1) + 2, # +2: instance_flow 
                device, 
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):  # generating motion
                if self.interrupt:
                    continue
        ...
        if not output_type == "latent":
            ...
            half_f = latents.shape[1] // 2
            print("additional_frames:", additional_frames, "half_f:", half_f)
            seg_latent = latents[:, additional_frames:half_f, :, :, :].to(self.vae.dtype)
            flow_latent = latents[:, half_f+additional_frames:, :, :, :] .to(self.vae.dtype)

            video_seg = self.decode_latents(seg_latent)  # torch.Size([1, 8, 16, 60, 90])
            video_flow = self.decode_latents(flow_latent)

            video = torch.cat([video_seg, video_flow], dim=3)
