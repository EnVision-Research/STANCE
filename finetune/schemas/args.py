...
class Args(BaseModel):
    ...
    video_column: Path
    stride: int = 1  # stride for sampling frames from videos, default is 1 (no stride)

    ########## Training #########
    resume_from_checkpoint: Path | None = None
    load_pretrained_weight: Path | None = None
    ...
    def parse_args(cls):
        ...
        parser.add_argument("--rgb_only", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument("--use_forcing", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument("--use_mass", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument("--use_depth", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument('--average_ins_flow', type=lambda x: x.lower() == 'true', default=False)
        ...
        parser.add_argument("--load_pretrained_weight", type=str, default=None)
        
        # Validation
        parser.add_argument("--do_validation", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument("--is_validation", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument("--validation_perturb", type=lambda x: x.lower() == 'true', default=False)
